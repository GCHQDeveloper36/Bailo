...
import { countReviews, findReviews } from '../../../services/review.js'
...
export const getReviewSummary = [
  ...
  async (req: Request, res: Response<GetReviewSummaryResponse>) => {
    ...
    const {
      query: { mine, modelId, semver, accessRequestId, kind, status },
    } = parse(req, getReviewSummarySchema)

    const reviewSummary = await countReviews(req.user, mine, modelId, semver, accessRequestId, kind, status)
