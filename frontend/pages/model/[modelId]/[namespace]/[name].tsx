import { useGetRemoteModel } from 'actions/model'
...
// This is a starting copy of the existing Model page.

export default function PeerModel() {
  const router = useRouter()
  const { modelId, namespace, name }: { modelId?: string; namespace?: string; name?: string } = router.query
  const peerId = modelId
  const combinedModelId = namespace + '/' + name

  const { model, isModelLoading, isModelError } = useGetRemoteModel(combinedModelId, EntryKind.MODEL, peerId)
  ...
  function requestAccess() {
    router.push(`/model/${peerId}/${namespace}/${name}/access-request/schema`)
  }
